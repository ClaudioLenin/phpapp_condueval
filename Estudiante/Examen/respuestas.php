...
class Respuestas {
    ...
    function obtenerNotaTotal($codestudiante, $codexamen) {
        try {
            Conexion::abrir_conexion();
            $sql = "SELECT * FROM tnotaexamen WHERE codme = "
                    . "(SELECT codme FROM tmateriaexamen "
                    . "INNER JOIN testudianteparalelo "
                    . "ON tmateriaexamen.codestudiante = testudianteparalelo.codestudianteparalelo "
                    . "WHERE testudianteparalelo.codestudiante = :codestudiante AND tmateriaexamen.codexamen = :codexamen)";
            $sentencia = Conexion::obtener_conexion()->prepare($sql);
            $sentencia->bindParam(":codestudiante", $codestudiante, PDO::PARAM_STR);
    ...
    function respuestasEstudiante($codestudiante, $codexamen) {
        Conexion::abrir_conexion();
        $sql = "SELECT * FROM testudianterespuesta WHERE codme = (SELECT codme FROM tmateriaexamen INNER JOIN testudianteparalelo
        ON tmateriaexamen.codestudiante = testudianteparalelo.codestudianteparalelo
        WHERE testudianteparalelo.codestudiante = :codestudiante AND tmateriaexamen.codexamen = :codexamen)";
        $sentencia = Conexion::obtener_conexion()->prepare($sql);
        $sentencia->bindParam(":codestudiante", $codestudiante, PDO::PARAM_STR);
